import re, json, arxiv, yaml, logging, argparse, datetime, requests, requests
...
def load_config(config_file: str) -> dict:
    ...
    def pretty_filters(**a) -> dict:
        ...
        def parse_filters(filters: list):
            ...
            for f in filters:
                if len(f.split()) > 1:
                    ret += excape_sig + f + excape_sig
                else:
                    ret += quota_sig + f + quota_sig
                if filters.index(f) != len(filters) - 1:
                    ret += or_sig
            return ret

        for k, v in a["keywords"].items():
            keywords[k] = parse_filters(v["filters"])
        return keywords

    with open(config_file, "r", encoding="utf8") as f:
        ret = yaml.load(f, Loader=yaml.FullLoader)
        ret["kv"] = pretty_filters(**ret)
        logging.info("config = %s", ret)
    return ret
...
def get_code_link(qword: str) -> str:
    ...
    r = requests.get(GITHUB_URL, params=params, timeout=60)
    results = r.json()
    code_link = None
...
def get_daily_papers(topic, query="slam", max_results=2, interesting_keywords:dict=None):
    """
    @param topic: str
    @param query: str
    ...
    for result in search_engine.results():
        ...
        activated_keywords : list = []
        if interesting_keywords is not None:
            for kwg in interesting_keywords:
                for kw in interesting_keywords[kwg]:
                    if kw.lower() in paper_title.lower() or kw.lower() in paper_abstract.lower():
                        activated_keywords.append(kwg)
                        break
        logging.info("Time = %s title = %s author = %s", update_time, paper_title, paper_first_author)
    ...
    with open(filename, "r") as f:
        ...
        for keywords, v in json_data.items():
            ...
            for paper_id, contents in v.items():
                ...
                valid_link = False if "[code-na]" in contents else True
                if valid_link:
                    continue
                try:
                    code_url = BASE_URL + paper_id  # TODO
                    ...
                    if "official" in r and r["official"]:
                        ...
                        if repo_url is not None:
                            new_cont = contents.replace("[code-na]", f"**[code]({repo_url})**")
...
def json_to_md(filename, md_filename, task="", to_web=False, use_title=True, use_tc=True, show_badge=True):
    ...
    with open(md_filename, "a+") as f:
        ...
        for keyword in data.keys():
            ...
            if use_title == True:
                if to_web == False:
                    f.write("|Publish Date|Title|Authors|Method Description|\n" + "|---|---|---|---|\n")
                else:
                    f.write("| Publish Date | Title | Authors | Method Description |\n")
                    f.write("|:---------|:-----------------------|:---------|:------|\n")
...
def demo(**config):
    ...
    if config["update_paper_links"] == False:
        ...
        for topic, keyword in keywords.items():
            ...
            data, data_web = get_daily_papers(topic, query=keyword, max_results=max_results, interesting_keywords=config["keywords"][topic]["interesting_keywords"])
